      ...
      value: chain ? (
        <span className="inline-flex items-center gap-2">
          ...
          {chain.chainId !== undefined && (
            <span className="text-slate-500 text-xs">· id {chain.chainId}</span>
          )}
        </span>
      ) : (
        <span className="text-amber-300">Unrecognized network</span>
      ),
    },
      ...
      <dl className="space-y-3">
        {rows.map((r) => (
          <div key={r.label} className="flex items-start justify-between gap-4">
            <dt className="text-xs uppercase tracking-wider text-slate-500 pt-0.5 shrink-0">
              {r.label}
            </dt>
              ...
              <div key={c.chain.id} className="p-3.5 rounded-xl bg-white/[0.03]">
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <div className="text-sm font-semibold shrink-0 tabular-nums">
                    {c.error ? (
                      <span className="text-red-300 text-xs font-normal">unavailable</span>
                    ) : (
                ...
                {c.error ? (
                  <p className="text-xs text-red-300/80">{c.error}</p>
                ) : (
                  <>
    ...
    <Panel
      ...
      action={
        href && transfers.length > 0 ? (
          <Link href={href} className="text-xs text-arc-400 hover:text-arc-300">
            View all →
          </Link>
        ) : undefined
      }
            ...
            const row = (
              <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/[0.04] transition-colors">
                ...
                >
                  ...
                  <span className="text-xs font-normal text-slate-400">{t.symbol}</span>
                </div>
              </div>
            );
            ...
            return (
              ...
              >
                ...
                {s.ok ? (
                  ...
                  <span
                    className="text-[11px] text-red-300/90 shrink-0 max-w-[45%] truncate"
                    title={s.error}
                  >
                    {s.error ?? 'unreachable'}
                  </span>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-slate-500 truncate">via {r.sourceChain}</div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  {dev === null ? (
                    ...
                    <div className="text-[11px] text-slate-500">not USD-pegged</div>
                  ) : (
                    <div
                      className={`text-[11px] tabular-nums ${
                        offPeg ? 'text-amber-300' : 'text-slate-500'
                      }`}
